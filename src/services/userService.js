...
class UserService {
    ...
    async service_query_select_by_func(funcs) {
        logger.info("UserService: Fetching users by functions", { funcs });
        
        try {
            const result = await query_select_by_func(funcs);
            if (!result) {
                logger.warn("UserService: No users found with specified functions", { funcs });
                return { status: 404, message: "user not found." };
            };
            logger.info(`UserService: Successfully fetched ${result.length || 0} users by functions`, { funcs });
            return result;
        } catch (error) {
            logger.error("UserService: Error fetching users by functions", { funcs, error: error.message });
            throw new Error(error.message);
        };
    };

    async service_query_insert_user(data) {
        logger.info("UserService: Creating new user", { username: data.username, email: data.email, func: data.func });
    ...
    async service_query_update_user_by_id(user_id, data) {
        logger.info("UserService: Updating user by ID", { user_id, username: data.username, email: data.email, func: data.func });
    ...
    async service_query_insert_notify_id(user_id, notify_id) {
        logger.info("UserService: Inserting notification ID", { user_id, notify_id });

        try {
            const user_if_exists = await query_select_by_id(user_id);

            if (!user_if_exists[0]?.user_id) {
                logger.warn("UserService: User not found for notification ID insertion", { user_id });
                return { status: 404, message: "user not found." };
            }

            const result = await query_insert_notify_id(user_id, notify_id);
            logger.info("UserService: Notification ID inserted successfully", { user_id, notify_id });
            return result;
        } catch (error) {
            logger.error("UserService: Error inserting notification ID", { user_id, notify_id, error: error.message });
            throw new Error(error.message);
        };
    };
};
